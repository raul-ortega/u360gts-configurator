...
TABS.firmware_flasher.initialize = function (callback) {
    ...
    $('#tabFirwareFlasher').load("./firmware_flasher.html", function () {
        // translate to user-selected language
        //localize();

        function enable_load_online_button() {
            $(".load_remote_file").text("Load Firmware [OnLine]").removeClass('disabled');
        }

        function parse_hex(str, callback) {
            // parsing hex in different thread
            var worker = new Worker('./hex_parser.js');
        ...
        var buildBoardOptions = function(){
            ...
            boards_e.append($("<option value='0'>Chose board</option>"));

            var versions_e = $('select[name="firmware_version"]').empty();
            versions_e.append($("<option value='0'>Chose version</option>"));
                ...
                .forEach(function(target, i) {
                    ...
                    descriptors.forEach(function(descriptor){
                        if($.inArray(target, selectTargets) == -1) {
                            ...
                            var select_e =
                                    $("<option value='" + descriptor.target +"'>" + descriptor.target +"</option>").data('summary', descriptor);
        ...
        $.get('https://api.github.com/repos/raul-ortega/u360gts/releases', function (releasesData){
            ...
            $('select[name="board"]').change(function() {
                ...
                if (!GUI.connect_lock) {
                    ...
                    var versions_e = $('select[name="firmware_version"]').empty();
                    if(target == 0) {
                        versions_e.append($("<option value='0'>Select firmware version</option>"));
                    } else {
                        versions_e.append($("<option value='0'>Choose a Firmware version for " + target +"</option>"));
                    }

                    TABS.firmware_flasher.releases[target].forEach(function(descriptor) {
                        var select_e =
                                $("<option value='" + descriptor.version + "'>" + descriptor.version + " - " + descriptor.target + " - " + descriptor.date + " (" + descriptor.status + ")</option>").data('summary', descriptor);
            ...
            if (!$(this).hasClass('disabled')) {
                if (!GUI.connect_lock) { // button disabled while flashing is in progress
                    if (parsed_hex != false) {
                        ...
                        if (String($('#port-picker').val()) != 'DFU') {
                            if (String($('#port-picker').val()) != '0') {
                                var port = String($('#port-picker').val()),
                                    baud;
                                ...
                                if ($('input.updating').is(':checked')) {
                                    ...
                                    options.reboot_baud = parseInt($('#baud-picker').val());
                                }
...
function operating_system(){
    ...
    else if (navigator.appVersion.indexOf("CrOS") != -1)    return "ChromeOS";
